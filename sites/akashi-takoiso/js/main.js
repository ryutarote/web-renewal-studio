...
(function () {
  ...
  const DESC = {
    home:  "明石名物・玉子焼（明石焼）の「たこ磯」（リニューアル提案）。魚の棚商店街の人気店。卵をたっぷり使ったふわとろの生地に、明石のタコ。あたたかい出汁にひたして味わう、明石の粉もんです。",
    menu:  "たこ磯のお品書き。玉子焼（明石焼）、あなご入り、ミックス焼など。価格は税込・目安です。明石・魚の棚。",
    story: "たこ磯の歩み。明石の魚の棚商店街で、特注の銅板で焼く玉子焼（明石焼）を提供する人気店。",
    shop:  "明石焼 たこ磯の店舗情報・アクセス。兵庫県明石市本町（魚の棚商店街）。JR明石駅すぐ。",
    order: "たこ磯のお取り寄せ（提案デモ）。明石焼の全国発送など、家庭で味わう構成例。"
  };
  const views = $$(".view");
  ...
  function renderCart() {
    ...
    if (ids.length === 0) {
      cartItemsEl.innerHTML = `<div class="cart-empty"><p>カートは空です。<br>明石焼の発送はいかがですか？</p><a class="btn btn-primary" href="#/order">お取り寄せを見る</a></div>`;
    } else {
