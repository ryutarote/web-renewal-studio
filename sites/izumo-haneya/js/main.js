...
(function () {
  ...
  const DESC = {
    home:  "島根・出雲そばの老舗「献上そば 羽根屋」（リニューアル提案）。江戸末期創業、皇室に献上した伝統を重んじる手打ちそば。本店は出雲市今市町。割子そば・釜揚げそば。冷凍生そば等のお取り寄せも。",
    menu:  "献上そば 羽根屋のお品書き。名物の割子そば、釜揚げそば、天ぷら・季節の一品。出雲そばならではのコシと香り。価格は税込・目安です。",
    story: "羽根屋の物語。江戸末期に創業し、明治の行幸では天皇に蕎麦を献上。以来「献上そば」と称し、代々受け継いだ伝統を重んじる出雲そばの老舗。",
    shop:  "献上そば 羽根屋の店舗情報・会社概要。島根県出雲市今市町本町549／0853-21-0058。営業 11:00〜19:30（中休みあり）。特定商取引法に基づく表記。",
    order: "羽根屋のお取り寄せ。出雲そば（半生・つゆ付）、釜揚げそば用 生そば、進物用 化粧箱。ご家庭で本格出雲そばを。アカウント登録不要のゲスト購入OK（デモ）。"
  };
  const views = $$(".view");
  ...
  function renderCart() {
    ...
    if (ids.length === 0) {
      cartItemsEl.innerHTML = `<div class="cart-empty"><p>カートは空です。<br>出雲そばのお取り寄せはいかがですか？</p><a class="btn btn-primary" href="#/order">お取り寄せを見る</a></div>`;
    } else {
